...
function newGuid(format){
        return Guid.NewGuid().ToString(format);
    }
function  funcSign(timestamp, nonce, application, version, body) {
...
function returnData(json){
    // '{"SearchKeyword":" 0620","UniversityCode":"","MajorCode":"","PageIndex":1,"PageSize":30}'
    let body = json;
    let timestamp = Math.floor((new Date()).getTime() / 1000);
    let nonce = newGuid();
    let application = 'Pdfreader.Web';
    let version = 'V2.2';
    let sign = funcSign(timestamp, nonce, application, version, body);
    return {
        'timestamp': timestamp,
        'nonce': nonce,
        'sign': sign,
